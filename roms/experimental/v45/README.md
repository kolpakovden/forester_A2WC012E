...
Оригинальный BIN **побайтно проверен локально**, но сам 524288-байтный payload пока не находится в Git tree.

Причина техническая: текущий GitHub connector принимает текстовые файлы и небольшие binary blobs, но при попытке передать этот ROM через промежуточное binary transport-представление payload был обрезан. Обрезанный артефакт удалён из репозитория и не используется.

Поэтому эталоном для будущей загрузки остаётся именно предоставленный файл и его SHA-256:

```text
92e0ff671bda35fe7951d68513c2ed188035b950556ffbe3e1e7cb603f241329
```

После физической загрузки `.bin` в репозиторий его нужно повторно скачать и проверить этим SHA-256.
